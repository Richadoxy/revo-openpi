"""Training configs for Revo pi05 fine-tuning."""
...
class AssetsConfig:
    """Location of normalization stats and other data assets."""
...
class ModelTransformFactory(GroupFactory):
    """Creates model transforms for pi0/pi05 models."""
...
@dataclasses.dataclass(frozen=True)
class LeRobotRevoDataConfig(DataConfigFactory):
    """Data config for Revo bimanual Revo3 LeRobot v2.1 datasets.

    The current dataset stores 70D state/action vectors. RevoInputs drops the first 14 pose dimensions and trains
    pi05 on the 56D joint-only action space:
    left arm 7 + right arm 7 + left hand 21 + right hand 21.
    """
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[revo_policy.RevoInputs(model_type=model_config.model_type)],
            outputs=[revo_policy.RevoOutputs()],
        )

        if self.extra_delta_transform:
            delta_action_mask = _transforms.make_bool_mask(7, 7, -21, -21)
