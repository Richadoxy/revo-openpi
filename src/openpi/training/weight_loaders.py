...
class PartialCheckpointWeightLoader(WeightLoader):
    """Loads checkpoint weights while keeping shape-mismatched action layers randomly initialized.

    This is useful when fine-tuning a pi05 base checkpoint with a different action dimension. For Revo, the
    released pi05 base checkpoint uses 32D actions, while the Revo3 bimanual joint action space is 56D. Most of the
    vision/language/action-expert backbone remains shape-compatible; action projection layers do not.
    """
    ...
    def load(self, params: at.Params) -> at.Params:
        ...
        for k, v in flat_loaded.items():
            ...
            if ref_shape == loaded_shape:
                ...
                raise ValueError(
                    ...
                    f"Layer does not match skip_on_mismatch_regex={self.skip_on_mismatch_regex!r}."
                )
        ...
        if skipped_keys:
            logger.warning(
                f"Partially loaded checkpoint: skipped {len(skipped_keys)} shape-mismatched layers. "
